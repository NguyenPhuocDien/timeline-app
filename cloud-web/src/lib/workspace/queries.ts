...
async function getWorkspaceStats(
  ...
  workspaceId: string,
): Promise<{ stats: WorkspaceStats; issues: string[] }> {
  const now = new Date().toISOString();
  const dueSoonEnd = hoursFromNowIso(48);
  ...
  const focusRows =
    (focusMinutesResult.data as { actual_minutes: number | null }[] | null) ??
    [];
  ...
  return {
    stats: {
      ...
      focusMinutesThisWeek: focusRows.reduce(
        (sum, row) => sum + (row.actual_minutes ?? 0),
        0,
      ),
    ...
    issues: [
      ...
      dueSoonTasksResult.error?.message,
      scheduledEventsResult.error?.message,
      focusMinutesResult.error?.message,
...
export async function getWorkspaceSnapshot(
  ...
  try {
    ...
    const statsResult = await getWorkspaceStats(supabase, workspaceId);
