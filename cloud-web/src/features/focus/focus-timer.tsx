...
export function FocusTimer({
  ...
  function buildSnapshot(currentStatus: TimerStatus): SessionSnapshot {
    const actualSeconds = computeActualSeconds(currentStatus);
    return {
    ...
    intervalRef.current = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          setTimeout(() => {
            const snap = buildSnapshot("running");
            setSession(snap);
            setTimerStatus("done");
          }, 0);
          return 0;
  ...
  function handleEndSession() {
    clearTick();
    const snap = buildSnapshot(timerStatus);
    setSession(snap);
    setTimerStatus("done");
  }
