...
export function WorkspaceShell({ snapshot }: WorkspaceShellProps) {
  ...
  const isReadOnly =
    snapshot.mode !== "live" || snapshot.membershipRole === "viewer";
  ...
  const hasRecentLimitNotice =
    snapshot.mode === "live" &&
    (snapshot.tasks.length >= 12 ||
      snapshot.events.length >= 8 ||
      snapshot.focusSessions.length >= 12 ||
      snapshot.labels.length >= 12);
    ...
    <>
      <main className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-6 py-8 md:px-10">
        <div className="glass rounded-[36px] border border-white/10 px-6 py-6 md:px-8 md:py-8">
          ...
          {hasRecentLimitNotice && (
            <div className="mt-6 rounded-3xl border border-cyan-300/16 bg-cyan-300/10 px-4 py-4 text-sm leading-6 text-cyan-50">
              This dashboard shows recent workspace records for speed. The top stats are calculated from the full workspace, while search filters the loaded recent records.
            </div>
          )}
